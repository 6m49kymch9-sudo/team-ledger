...
import { Plus, Search, Calendar, ImageIcon, TrendingUp } from 'lucide-react'
...
export default function IncomesPage() {
  ...
  const filteredIncomes = incomes.filter((income) => {
    ...
    const matchesMember =
      memberFilter === 'all' || income.user_id === memberFilter

    return matchesSearch && matchesCategory && matchesMember
  })

  const groupedIncomes = filteredIncomes.reduce(
    (groups, income) => {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            共 {filteredIncomes.length} 笔收入，合计 ¥{totalIncome.toLocaleString()}
          </p>
        </div>
        ...
        <Card className="p-8 text-center">
          <CardContent className="p-0">
            ...
            <p className="text-muted-foreground mb-4">
              {incomes.length === 0 ? '暂无收入记录' : '没有找到匹配的收入'}
            </p>
            {incomes.length === 0 && (
              <Link href="/app/incomes/new">
                <Button className="bg-green-600 hover:bg-green-700">记一笔收入</Button>
              </Link>
            )}
          </CardContent>
        </Card>
                  ...
                  {monthIncomes.map((income) => (
                    ...
                    >
                      <Avatar className="w-10 h-10">
                        <AvatarImage src={income.user?.avatar_url || ''} />
                        <AvatarFallback>
                          {income.user?.nickname?.[0] || 'U'}
                        </AvatarFallback>
                      </Avatar>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <Badge variant="outline" className="border-green-500 text-green-600">
                            {INCOME_CATEGORIES[income.category as keyof typeof INCOME_CATEGORIES]}
                          </Badge>
                          <span className="text-sm text-muted-foreground">
                            {income.user?.nickname || '未知'}
                          </span>
                          {income.image_url && (
